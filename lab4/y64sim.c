...
long_t compute_alu(alu_t op, long_t argA, long_t argB)
{
    long_t val = 0;
    switch(op) {
        case A_ADD: val = argA + argB; break;
        case A_SUB: val = argB - argA; break;
        case A_AND: val = argA & argB; break;
        case A_XOR: val = argA ^ argB; break;
        default: break;
    }
    return val;
}
...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    if(op == A_ADD || op == A_SUB) {
        bool_t signA = ((int)argA < 0);
        bool_t signB = ((int)argB < 0);
        if( (op == A_ADD && signA == signB && sign != signB)
         || (op == A_SUB && signA != signB && sign != signB) )
            ovf = TRUE;
    }
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
    bool_t doit = FALSE;
    switch(cond) {
      case C_YES:                                                doit = TRUE; break;
      case C_LE:  if((GET_SF(cc) ^ GET_OF(cc)) || GET_ZF(cc))    doit = TRUE; break;
      case C_L:   if(GET_SF(cc) ^ GET_OF(cc))                    doit = TRUE; break;
      case C_E:   if(GET_ZF(cc))                                 doit = TRUE; break;
      case C_NE:  if(!GET_ZF(cc))                                doit = TRUE; break;
      case C_GE:  if(!(GET_SF(cc) ^ GET_OF(cc)))                 doit = TRUE; break;
      case C_G:   if(!(GET_SF(cc) ^ GET_OF(cc)) && !GET_ZF(cc))  doit = TRUE; break;
    }
    return doit;
}
...
stat_t nexti(y64sim_t *sim)
{
    ...
    byte_t regs = 0;
    switch(icode) {
      case I_RRMOVQ: case I_RMMOVQ: case I_IRMOVQ: case I_MRMOVQ:
      case I_ALU: case I_PUSHQ: case I_POPQ:
    ...
    switch(icode) {
      case I_IRMOVQ: case I_RMMOVQ: case I_MRMOVQ: case I_JMP: case I_CALL:
        get_long_val(sim->m, next_pc, &imme);
        next_pc += 8;
        break;
      default: break;
    }
    ...
    switch (icode) {
      ...
      case I_RRMOVQ:  /* 2:x regA:regB */
        ...
        if(cond_doit(sim->cc, GET_COND(codefun)))
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        valA = get_reg_val(sim->r, regA);
        valB = get_reg_val(sim->r, regB);
        set_long_val(sim->m, imme + valB, valA);
        sim->pc = next_pc;
        break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        valB = get_reg_val(sim->r, regB);
        if(get_long_val(sim->m, imme + valB, &val))
            set_reg_val(sim->r, regA, val);
        else {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, imme + valB);
            return STAT_ADR;
        }
        sim->pc = next_pc;
        break;
      case I_ALU: /* 6:x regA:regB */
        ...
        break;
      case I_JMP: /* 7:x imm */
        sim->pc = ((cond_doit(sim->cc, GET_COND(codefun)))? imme : next_pc);
        break;
      case I_CALL: /* 8:x imm */
        val = get_reg_val(sim->r, REG_RSP);
        if(!set_long_val(sim->m, val - 8, next_pc)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, val - 8);
            set_reg_val(sim->r, REG_RSP, val - 8);
            return STAT_ADR;
        }
        set_reg_val(sim->r, REG_RSP, val - 8);
        sim->pc = imme;
      ...
      case I_RET: /* 9:0 */
        val = get_reg_val(sim->r, REG_RSP);
        get_long_val(sim->m, val, &(sim->pc));
        set_reg_val(sim->r, REG_RSP, val + 8);
        break;
      case I_PUSHQ: /* A:0 regA:F */
        valA = get_reg_val(sim->r, regA);
        val = get_reg_val(sim->r, REG_RSP);
        if(!set_long_val(sim->m, val - 8, valA)) {
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, val - 8);
            set_reg_val(sim->r, REG_RSP, val - 8);
            return STAT_ADR;
        }
      ...
      case I_POPQ: /* B:0 regA:F */
    	val = get_reg_val(sim->r, REG_RSP);
        set_reg_val(sim->r, REG_RSP, val + 8);
        get_long_val(sim->m, val, &imme);
        set_reg_val(sim->r, regA, imme);
        sim->pc = next_pc;
    	break;
      default:

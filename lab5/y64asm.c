...
{
    symbol_t* sym_ptr = symtab->next;
    while(sym_ptr != NULL) {
        if(!strcmp(sym_ptr->name, name))    return sym_ptr;
...
{
    /* check duplicate */
    symbol_t* sym_ptr = symtab;
    while(sym_ptr->next != NULL) {
        if(!strcmp(sym_ptr->next->name, name))  return -1;
        sym_ptr = sym_ptr->next;
    }
    /* create new symbol_t (don't forget to free it)*/
    /* add the new symbol_t to symbol table */
    sym_ptr = sym_ptr->next = (symbol_t*)malloc(sizeof(symbol_t));
    sym_ptr->name = (char*)malloc(strlen(name));
...
void add_reloc(char *name, bin_t *bin)
{
    ...
    reloc_t* reloc_ptr = (reloc_t*)malloc(sizeof(reloc_t));
    reloc_ptr->name = (char*)malloc(strlen(name));
    strcpy(reloc_ptr->name, name);
    reloc_ptr->y64bin = bin;
    /* add the new reloc_t to relocation table */
    reloc_ptr->next = reltab->next;
    reltab->next = reloc_ptr;
}
...
parse_t parse_instr(char **ptr, instr_t **inst)
{
    ...
    char instruction[7];
    int instr_len = 0;
    while(IS_LETTER(cur) || *cur == '.')  { ++instr_len; ++cur; }
    if(instr_len > 6)   return PARSE_ERR;
    strncpy(instruction, cur - instr_len, instr_len);
    instruction[instr_len] = '\0';
    /* set 'ptr' and 'inst' */
    *ptr = cur;
    *inst = find_instr(instruction);
    if(*inst == NULL)   return PARSE_ERR;
    else                return PARSE_INSTR;
}
...
parse_t parse_delim(char **ptr, char delim)
{
    ...
    /* set 'ptr' */
    *ptr = cur + 1;
    if(*cur == delim)   return PARSE_DELIM;
    err_print("Invalid '%c'", delim);
    return PARSE_ERR;
}
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    char reg[5];
    int regNameLen = 0;
    while(*cur == '%' || IS_LETTER(cur) || IS_DIGIT(cur))   { ++cur; ++regNameLen; }
    strncpy(reg, cur - regNameLen, regNameLen);
    reg[regNameLen] = '\0';
    const reg_t* reg_ptr = find_register(reg);
    if(reg_ptr == NULL) {
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    char* cur = *ptr;
    SKIP_BLANK(cur);
    if(!IS_LETTER(cur))     return PARSE_ERR;
    /* allocate name and copy to it */
    int nameLen = 0;
    for(nameLen = 0; IS_LETTER(cur) || IS_DIGIT(cur); ++nameLen, ++cur);
    *name = (char*)malloc(sizeof(char) * nameLen + 1);
    strncpy(*name, cur - nameLen, nameLen);
    (*name)[nameLen] = '\0';
    /* set 'ptr' and 'name' */
    *ptr = cur;
    return PARSE_SYMBOL;
...
parse_t parse_digit(char **ptr, int64_t *value)
{
    ...
    unsigned long tmp = strtoul(cur, &endPtr, 0);
    *value = tmp;
    /* set 'ptr' and 'value' */
    *ptr = endPtr;
    if(sign)    *value = -(*value);
...
parse_t parse_label(char **ptr, char **name)
{
    ...
    /* set 'ptr' and 'name' */
    *name = (char*)malloc(label_length + 1);
    strncpy(*name, cur - label_length, label_length);
    (*name)[label_length] = '\0';
    *ptr = cur + 1;
    return PARSE_LABEL;
}

void little_endian_imme(byte_t* dest, int64_t value, int len)
{
    int i;
    for (i = 0; i < len; i++) {
        dest[i] = (value & 0xFF);
...
*           call SUM  #invoke SUM function */
    char* cur = line->y64asm;
    /* skip blank and check IS_END */
    SKIP_BLANK(cur);
    if(IS_END(cur))       { line->type = TYPE_COMM; return line->type; }

    /* is a comment ? */
    if(IS_COMMENT(cur))   { line->type = TYPE_COMM; return line->type; }

    /* is a label ? */
    char* labelName;
    if(parse_label(&cur, &labelName) == PARSE_LABEL) {
        if(add_symbol(labelName, vmaddr) < 0) {
            line->type = TYPE_ERR;
            err_print("Dup symbol:%s", labelName);
            return line->type;
        };
        SKIP_BLANK(cur);
        if(IS_END(cur) || IS_COMMENT(cur)) {
            line->type = TYPE_INS;
    ...
    /* is an instruction ? */
    instr_t* instr;
    if(parse_instr(&cur, &instr) == PARSE_ERR)  { line->type = TYPE_ERR; return line->type; }
    ...
    switch(icode) {
        ...
        case I_RRMOVQ: 
            ...
            || parse_delim(&cur, ',') == PARSE_ERR
            || parse_reg(&cur, &regB) == PARSE_ERR)
                    line->type = TYPE_ERR;
            else    (line->y64bin).codes[1] = HPACK(regA, regB);
            break;
        case I_IRMOVQ: 
            if((valC_return = parse_imm(&cur, &name, &valC)) == PARSE_ERR 
            || parse_delim(&cur, ',') == PARSE_ERR
            || parse_reg(&cur, &regB) == PARSE_ERR) {
                    line->type = TYPE_ERR;
            } else {
                (line->y64bin).codes[1] = HPACK(REG_NONE, regB);
                if(valC_return == PARSE_DIGIT) {
                    little_endian_imme((line->y64bin).codes + 2, valC, 8);
                } else {
        ...
        case I_RMMOVQ: 
            if(parse_reg(&cur, &regA) == PARSE_ERR 
            || parse_delim(&cur, ',') == PARSE_ERR
            || parse_mem(&cur, &valC, &regB) == PARSE_ERR) {
                line->type = TYPE_ERR;
            } else {
            ...
            break;
        case I_MRMOVQ:
            if(parse_mem(&cur, &valC, &regB) == PARSE_ERR
            || parse_delim(&cur, ',') == PARSE_ERR
            || parse_reg(&cur, &regA) == PARSE_ERR ) {
                line->type = TYPE_ERR;
            } else {
                (line->y64bin).codes[1] = HPACK(regA, regB);
                little_endian_imme((line->y64bin).codes + 2, valC, 8);
        ...
        case I_ALU: 
            ...
            break;
        case I_JMP:
            valC_return = parse_imm(&cur, &name, &valC);
            if(valC_return == PARSE_ERR) {
                err_print("Invalid DEST");
                line->type = TYPE_ERR;
            } else if(valC_return == PARSE_DIGIT) {
                little_endian_imme((line->y64bin).codes + 1, valC, 8);
            } else {
                add_reloc(name, &(line->y64bin));
            }
            break;
        ...
        case I_RET: break;
        case I_PUSHQ: 
            if(parse_reg(&cur, &regA) == PARSE_ERR)     line->type = TYPE_ERR;
            else    (line->y64bin).codes[1] = HPACK(regA, REG_NONE);
            break;
        case I_POPQ: 
            if(parse_reg(&cur, &regA) == PARSE_ERR)     line->type = TYPE_ERR;
            else    (line->y64bin).codes[1] = HPACK(regA, REG_NONE);
        ...
        case I_DIRECTIVE:
            switch(ifun) {
                case D_ALIGN: 
                    if(parse_digit(&cur, &valC) == PARSE_ERR)  {
                        line->type = TYPE_ERR;
                    } else {
                        while(vmaddr % valC)    vmaddr++;
                        (line->y64bin).addr = vmaddr;
                    }
                    break;
                case D_POS: 
                    if(parse_digit(&cur, &valC) == PARSE_ERR)   line->type = TYPE_ERR;
                    else  {
                        (line->y64bin).addr = valC;
                        vmaddr = valC;
                    }
                    break;
                case D_DATA:
                    SKIP_BLANK(cur);
                    if(IS_DIGIT(cur) && parse_digit(&cur, &valC) != PARSE_ERR)  {
                        little_endian_imme((line->y64bin).codes, valC, (line->y64bin).bytes);
                    } else if(IS_LETTER(cur) && parse_symbol(&cur, &name) != PARSE_ERR) {
                        add_reloc(name, &(line->y64bin));
                    } else {
                        line->type = TYPE_ERR;
                    } 
...
int relocate(void)
{
    ...
    while (rtmp) {
        /* find symbol */
        symbol_t* sym_ptr = find_symbol(rtmp->name);
        if(sym_ptr == NULL) {
            err_print("Unknown symbol:'%s'", rtmp->name);
            return -1;
        }
        bin_t* bin_ptr = rtmp->y64bin;
        /* relocate y64bin according itype */
        little_endian_imme(bin_ptr->codes + bin_ptr->bytes - 8, sym_ptr->addr, 8);
        /* next */
        rtmp = rtmp->next;
    }
    return 0;
...
int binfile(FILE *out)
{
    ...
    int currentAddr = 0;
    char padding[10000] = {0};
    /* binary write y64 code to output file (NOTE: see fwrite()) */
    while(p != NULL) {
        if(p->type == TYPE_COMM)  { p = p->next; continue; }
        if(p->type == TYPE_ERR)   return -1;   
        if((p->y64bin).bytes) {
            if(currentAddr != (p->y64bin).addr)
                fwrite(padding, sizeof(byte_t), (p->y64bin).addr - currentAddr, out);
            fwrite((p->y64bin).codes, sizeof(byte_t), (p->y64bin).bytes, out);
            currentAddr = (p->y64bin).addr + (p->y64bin).bytes;
        }
        p = p->next;
    }
    return 0;
